...
import com.mot.upd.pcba.Util.DBUtil;
...
public class DispatchSerialNumberDAO {
	private static Logger logger = Logger.getLogger(DispatchSerialNumberDAO.class);
	...
	public DispatchSerialResponsePOJO updateDispatchStatusIMEI(
			DispatchSerialRequestPOJO dispatchSerialRequestPOJO, DispatchSerialResponsePOJO dispatchSerialResponsePOJO) {
		...
		try {
			...
			dispatchSerialResponsePOJO.setResponseMsg(ServiceMessageCodes.NO_DATASOURCE_FOUND_DISPATCH_SERIAL_MSG);
			return response;
		}
		try
		{
			...
			preparedStmt.setString(5, dispatchSerialResponsePOJO.getNewSerialNo().trim());
			...
			dispatchSerialResponsePOJO.setResponseCode(ServiceMessageCodes.SQL_EXCEPTION);
			dispatchSerialResponsePOJO.setResponseMsg(ServiceMessageCodes.SQL_EXCEPTION_MSG);
			try {
				con.rollback();
